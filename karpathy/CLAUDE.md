...
### 核心目标
维护 `wiki/` 目录，将其打造为**可复利增长的知识层**——知识被编译一次、持续保鲜，而非每次从零检索。

### 你的职责
- 创建和维护 `wiki/` 中的所有 Markdown 页面
- 当用户添加新来源时执行 **Ingest**（摄入）
- 当用户提问时执行 **Query**（基于 wiki 查询）
- 当用户要求时执行 **Lint**（健康检查）
- 维护 `wiki/index.md`（内容索引）和 `wiki/log.md`（操作日志）

### 边界
- ✅ 在 `wiki/` 中自由创建、修改 Markdown 页面
- ✅ 读取 `raw/` 中的原始资料（只读）
- ✅ 读取 `30 - 资源/` 中的现有内容（如微信读书笔记）供参考
- ❌ **绝不修改** `raw/` 中的任何文件
- ❌ 不要动 PARA 文件夹（`00 - 收件箱`、`10 - 日记`、`20 - 项目`、`40 - 存档`）
- ❌ 不要动 `.obsidian/` 配置
- ❌ 不要动 `blink/`、`karpathy/` 等参考文件夹
...
| 目录 | 用途 | 来源工具 |
|------|------|---------|
| `raw/articles/` | 网页文章（Obsidian Web Clipper 剪藏） | Obsidian Web Clipper 扩展 |
| `raw/books/` | 书籍笔记、微信读书划线 | weread-plugin 自动同步 |
| `raw/papers/` | 研究论文、报告 | 手动放入 |
| `raw/media/` | 图片、图表、截图 | 截图/下载 |
| `raw/assets/` | Obsidian 附件下载 | Obsidian "下载附件" 功能 |

### Wiki 知识层（`wiki/`，LLM 全权维护）
| 目录 | 用途 | 说明 |
|------|------|------|
| `wiki/sources/` | 单个来源的摘要页 | 一源一页，提炼核心要点 |
| `wiki/entities/` | 人物、书籍、项目、组织等实体页 | 跨来源积累的信息 |
| `wiki/concepts/` | 方法、理论、模型、框架 | 跨来源抽象的概念 |
| `wiki/comparisons/` | 对比分析页 | A vs B，相同/不同、选择建议 |
| `wiki/overview/` | 主题综述、综合页 | 一个领域的全景图 |

### 根目录文件
- `wiki/index.md`：内容索引（LLM 每次 Ingest 后更新，Query 时用作导航）
- `wiki/log.md`：操作日志（仅追加，记录每次操作的时间线）
...
**触发**：用户将文件放入 `raw/` 并说"请基于 `raw/xxx` 进行 Ingest"。

**流程**：
1. **阅读**：读取 `raw/xxx` 的完整内容。如果含图片（`![[image.png]]`），先读文本再逐一查看图片获取完整上下文。
2. **确认重点**：与用户简短讨论 3-5 条核心要点，确认哪些该强调。如果用户没有特别指令，自行判断。
3. **创建/更新来源摘要**：在 `wiki/sources/` 创建摘要页（文件名格式：`来源_文章标题.md`）。
4. **更新实体页**：识别来源中提到的人物、书籍、项目、组织，在 `wiki/entities/` 创建或更新对应页面。
5. **更新概念页**：识别方法、理论、模型，在 `wiki/concepts/` 创建或更新对应页面。
6. **更新索引**：在 `wiki/index.md` 对应分类表格中添加新页面条目（链接 + 一句话摘要）。
7. **追加日志**：在 `wiki/log.md` 末尾追加记录：
   ```
   ## [YYYY-MM-DD] ingest | raw/xxx → wiki/sources/来源_标题.md（+ 受影响页面列表）
   ```
8. **提议综合**：如果多个来源涉及同一主题，提议创建 `wiki/overview/` 综合页或 `wiki/comparisons/` 对比页。
...
**流程**：
1. **导航**：先读 `wiki/index.md` 找到候选页面，再读具体页面内容。
2. **综合回答**：基于页面内容综合答案，附上 `[[wikilink]]` 引用。
3. **归档有价值的回答**：如果回答是新的比较、分析或综合，**主动提议**将其归档为 wiki 页面（`wiki/comparisons/` 或 `wiki/overview/`），而非让答案消失在聊天历史中。
4. **记录日志**：如果创建了新页面，追加到 `wiki/log.md`：
   ```
   ## [YYYY-MM-DD] query | 新建 wiki/comparisons/A_vs_B.md
   ```

**回答的多种输出形式**（取决于问题类型）：
- Markdown 页面 → 归档到 wiki
- 对比表 → 归档到 `wiki/comparisons/`
...
4. **执行修复**：经确认后逐条修复，每次记录到 `wiki/log.md`：
   ```
...
- LLM Wiki 与 PARA 是**两个正交体系**：
  ...
  - LLM Wiki（`raw/` + `wiki/`）：管深度知识积累
- 两者可通过 wikilink 互连：比如 wiki 概念页可以引用 PARA 中的项目进度

### 与微信读书集成
- `30 - 资源/微信读书/` 中的笔记由 `weread-plugin` 自动同步，已有 YAML frontmatter
- 用户可能要求你基于这些笔记创建 wiki 页面——和 Ingest 流程一样，只是源文件在 `30 - 资源/` 而非 `raw/`
- 建议先复制到 `raw/books/` 再操作（保持 raw/ 的完整性），或直接在原位读取后写入 wiki
...
| 页面关系查询 | 直接搜索 `wiki/` 目录 | 目录结构浅，index.md 是主要导航工具 |
